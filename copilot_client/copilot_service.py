...
from .config import (AGENT_FILE, FALLBACK_MODELS, MAX_BUILD_FIX_ROUNDS, MAX_REPAIR_ROUNDS,
                     OUT_ROOT, PROMPT_TREE_BUDGET)
from .figma_prefetch import fetch_renders
from .file_writer import (render_attachments, save_renders, scan_new_files,
                          write_generated_files)
from .parser import (build_screen_payloads, collect_image_fills, curate_document, extract_screens_and_flow,
                     extract_tokens, summarize_node, tree_stats)
from .prompts import (build_fix_prompt, build_prompt_inline, build_prompt_local_mcp, build_prompt_mcp,
                      build_repair_prompt, build_visual_fix_prompt, fill_template,
                      load_context_template)
from .qa_engine import compute_metrics, low_scoring, run_cmd, visual_verify
...
async def pipeline(*, file, file_key, source, renders, figma_token, github_pat,
    ...
    try:
        ...
        async def send(p, atts=None, timeout=600):
            ...
            return getattr(data, "content", None) or getattr(resp, "content", None) or ""

        async def send_and_collect(p, atts=None, timeout=600):
            t_start = time.time()
            text = await send(p, atts, timeout)
        ...
        text, gen_files = await send_and_collect(prompt, attachments)
